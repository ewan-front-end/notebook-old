 ...
 *     id:'home_tools',          // 用于从flatDataMap中查询数据
 *     path:'绝对路径docs/tools' // 链接[name](path)
 * }}
 */
let flatDataMap = {}, // 第一次扁平化数据
...
const createFile = (item) => {
    ...
    if (!fs.existsSync(absolutePath)) {
        flatDataMap[id] = {type: 'DIRE', path: absolutePath, target: item}; 
        flatDataMap[id + '/index'] = {type: 'FILE', path: _path.resolve(absolutePath, 'README.md'), target: item}
    }
...
for (key in siteMap) {
    ...
    handleItem(key, item, {key:'ROOT_HOME', id: '', path: '.', title: '首页标题'})
    // 首页一级导航

...
[#]  密码 
    - [a-zA-Z\d@!%&\$\*\?]{8,}  // 大小写字母、数字、@!%&$*? 任意组合 8位以上
    - (?=.*[A-Z])               // 限制 某位置会出现大写字母
    - (?=.*[a-z])               // 限制 某位置会出现小写字母
    - (?=.*\d)                  // 限制 某位置会出现数字
    - (?=.*[@!%&\$\*\?]         // 限制 某位置会出现列举字符

    ^(?=.*[A-Za-z])(?=.*d)[A-Za-zd]{8,}$                                  // 至少8个字符，至少一个字母和一个数字
    ^(?=.*[A-Za-z])(?=.*d)(?=.*[@$!%*#?&])[A-Za-zd@$!%*#?&]{8,}$          // 至少8个字符，至少一个字母，一个数字和一个特殊字符
    ^(?=.*[a-z])(?=.*[A-Z])(?=.*d)[a-zA-Zd]{8,}$                          // 至少8个字符，至少一个大写字母，一个小写字母和一个数字
    ^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$  // 至少8个字符，至少一个大写字母，一个小写字母，一个数字和一个特殊字符
    ^(?=.*[a-z])(?=.*[A-Z])(?=.*d)(?=.*[@$!%*?&])[A-Za-zd@$!%*?&]{8,10}$  // 最少8个字符，最多10个字符，至少一个大写字母，一个小写字母，一个数字和一个特殊字符
===-
<pre class="fs16">
<strong>匹配位置</strong>
    ^、$、\b、\B
